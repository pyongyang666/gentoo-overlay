...
src_prepare() {
	...
	sed -e "s:#QMAKE_LRELEASE = /usr/bin/lrelease:QMAKE_LRELEASE = /usr/lib64/qt5/bin/lrelease:;s:#QMAKE_CXXFLAGS += -O3:QMAKE_CXXFLAGS += -O3:;s:#QMAKE_LEX = flex:QMAKE_LEX = flex:;s:#QMAKE_YACC = bison:QMAKE_YACC = bison:;s:#LIBZ_LIBS    = -lz:LIBZ_LIBS    = -lz:" src/gcconfig.pri.in > src/gcconfig.pri || die
